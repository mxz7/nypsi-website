...
<Main class="w-full px-4 lg:max-w-3xl lg:px-0">
  <div class="flex">
    <div class="flex-1">
  ...
  <section class="grid w-full grid-cols-2 gap-4">
    <Card>
      ...
      {#if data.status.load}
        <p class="text-sm">load: {data.status.load}</p>
      {/if}
    </Card>

    <Card>
      <h3 class="card-title {data.database.online ? 'text-success' : 'text-error'}">database</h3>

      <p class="text-sm">query: {data.database.latency.toFixed(2)}ms</p>
    </Card>
  </section>
