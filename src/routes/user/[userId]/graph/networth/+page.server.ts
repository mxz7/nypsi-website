...
export const load = async ({ params, parent, setHeaders }) => {
  ...
  if (params.userId !== parentData.user.id) throw redirect(302, `/user/${parentData.user.id}/graph/networth`);

  setHeaders({ "cache-control": "max-age=600" });

  return { base: 69, streamed: { graphData: getGraphData("user-net", params.userId), userId: params.userId } };
};
